(in-package :ccl)
;;; Only: (hi::buffer-modeline-fields buffer) — no field funcalls.
(objc:defmethod (#/drawRect: :void) ((self gui::modeline-view) (rect :<NSR>ect))
  ...
  (let* ((bounds (#/bounds self))
    ...
    (#_NSRectFill bot)
    (when buffer
      (let ((fields (hi::buffer-modeline-fields buffer)))
        (setq *last-built* (mapcar #'hi::modeline-field-name fields))))
