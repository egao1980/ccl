...
(defun %show-stack-frame-label (frame-number p context lfun pc detailed-p)
  ;; detailed-p NIL: never format stack args.  On darwinarm64 those slots
  ;; are often BOGUS; ~s → CLASS-OF → no-class-error, which turns a mild
  ;; callback error into a break on Initial (Hemlock sheet / our logger).
  (flet ((frame-label ()
